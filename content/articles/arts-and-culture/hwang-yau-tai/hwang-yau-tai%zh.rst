=======================
黃友棣音樂數位資料庫
=======================

:date: 2025-11-21
:modified: 2025-12-01
:tags: 黃友棣音樂數位資料庫, 黃友棣, 黃友棣教授, 樂風泱泱, 大樂必易
:category: 藝術-文化
:summary: 黃友棣音樂數位資料庫


.. list-table:: 黃友棣音樂數位資料庫
   :widths: 33 33 33 
   :header-rows: 1

   * - `樂風泱泱`_
     - `創作手稿`_
     - `專題報導`_

   * - `音樂作品集 <{filename}music-works%zh.rst>`_
     - `音樂作品精選 <{filename}music-works-special%zh.rst>`_
     - `出版及相關著作`_

   * - `影像寫真 <{filename}photo%zh.rst>`_ 
     - `相關網站連結 <{filename}related-website-links%zh.rst>`_
     - `留言版及其他`_

------

樂風泱泱
~~~~~~~~~~~

- `生命歴程 <{filename}magnificent-style-of-music/life-journey%zh.rst>`_
- `樂教推手 <{filename}magnificent-style-of-music/champion-of-music-education%zh.rst>`_
- `作品綜述 <{filename}magnificent-style-of-music/work-overview%zh.rst>`_
- `歴史之音 <{filename}magnificent-style-of-music/voices-of-history%zh.rst>`_
- `年表 <{filename}magnificent-style-of-music/chronology%zh.rst>`_

------

創作手稿
~~~~~~~~~~~

- `樂譜 <{filename}creative-manuscript/sheet-music%zh.rst>`_
- `書信手札 <{filename}creative-manuscript/manu-scripts-list%zh.rst>`_
- `作品解説 <{filename}creative-manuscript/works-description-list%zh.rst>`_ 

------

專題報導
~~~~~~~~~~~

- `新聞採録 <{filename}feature-report/news-brief-list%zh.rst>`_
- `春風化雨薪火相傳 <{filename}feature-report/torch-generation-to-generation%zh.rst>`_
- `與音樂教育家相遇 <{filename}feature-report/meeting-with-music-educators%zh.rst>`_

------

出版及相關著作
~~~~~~~~~~~~~~~~~~

- `樂教文集 <{filename}publication-and-refs/music-education-collection%zh.rst>`_
- `音樂論著 <{filename}publication-and-refs/music-treatises%zh.rst>`_
- `音樂著作 <{filename}publication-and-refs/music-publications%zh.rst>`_
- `有聲資料 <{filename}publication-and-refs/audio-materials%zh.rst>`_
- `作家著作 <{filename}publication-and-refs/writers-works%zh.rst>`_
- `學術論文 <{filename}publication-and-refs/academic-papers%zh.rst>`_

------

留言版及其他
~~~~~~~~~~~~~~~~

- `留言版 <{filename}bbs-etc/bbs%zh.rst>`_
- `資料共享 <{filename}bbs-etc/data-share%zh.rst>`_

------

取材自： 高雄市政府文化局 （ `頁面存檔備份 <https://web.archive.org/web/20100212170156/http://mim.khcc.gov.tw/hyl/1_1index.asp>`__ ，存於網際網路檔案館, 2010-02-12）


..
  12-01 rev. replace music-papers%zh.rst with sheet-music%zh.rst
  2025-11-21 post; 11-08 created rst; start on 2025-11-07
  (建構中！)
  資料庫檢索	MusicWorksGuide.aspx

		分類查詢	MusicWorksGuide.aspx
		進階查詢	MusicWorksSearch.aspx